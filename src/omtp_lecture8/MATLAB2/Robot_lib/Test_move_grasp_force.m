...
for i = 1:size(C,2)
    ...
    CloseHand(robot);
    pause(2)
    % Move down in global Z position unitl the desired forece is reached,
    % afterwards brake
    CmoveFor_Force([0,0,-0.3],2, time,robot)
    
    pause(2)
    OpenHand(robot);
    pause(2)
    
   % Move down in global Z position for 2 cm,
    CmoveFor([0,0,-0.02], time,robot)
